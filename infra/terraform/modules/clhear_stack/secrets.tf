...
# FINRA API client secret — pre-existing secret created in the console (like Anthropic).
data "aws_secretsmanager_secret" "finra_api" {
  count = trimspace(var.finra_api_secret_name) != "" ? 1 : 0
  name  = trimspace(var.finra_api_secret_name)
}
